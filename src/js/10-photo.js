...
//   1. shutter — the valley is rendered at K instants inside the t seconds
//      the shutter is open and averaged (motion blur, shake);
...
const NEAREST = 23;                           // cm: the front of the tray, the nearest thing in view
const photoCam = new THREE.PerspectiveCamera(30, 1.5, 4, 700);
photoCam.rotation.order = 'YXZ';
photoCam.layers.set(LAYER_PHOTO);
photoCam.layers.enable(LAYER_PHOTO_ONLY);
...
const packP = fsPass({
  ...
  uNear: { value: photoCam.near }, uFar: { value: photoCam.far }, uFocus: { value: 38 }, uK: { value: 1 }, uMax: { value: MAX_R }, uW: { value: 1 },
}, `
  uniform sampler2D tColor, tDepth;
  uniform float uNear, uFar, uFocus, uK, uMax, uW;
  varying vec2 vUv;
  void main() {
...
const sensorP = fsPass({
  ...
  void main() {
    ...
    vec3 e = max(lin * uExpo, 0.0) * uFWC;
    e += uNoise * gauss3(gl_FragCoord.xy, uSeed) * sqrt(e + uRead * uRead);
    vec3 raw = clamp(e / uFWC, 0.0, 1.0);
...
// 4 → what the camera's meter sees: block averages of the (un-exposed) light, log-encoded
...
// 5 → onto the screen, with the camera's display aids (the big viewer)
...
// ---------------------------------------------------------------------------
// Exposure calibration. A hidden 18 % grey card in the valley, facing the
// camera, is rendered on its own; all lights are then scaled so it reads
// exactly 0.18. After that, "correct exposure" means EV(setting) = scene EV.
// ---------------------------------------------------------------------------
const CAL_AT = V3(48, AXIS_Y - 3, 3);
const calCard = new THREE.Mesh(new THREE.PlaneGeometry(5, 5).rotateY(-Math.PI / 2), new THREE.MeshStandardMaterial({ color: new THREE.Color(0.18, 0.18, 0.18), roughness: 1, metalness: 0 }));
calCard.position.copy(CAL_AT);
calCard.layers.set(LAYER_CAL);
scene.add(calCard);
const calCam = new THREE.PerspectiveCamera(3, 1, 1, 200);
calCam.position.set(0, AXIS_Y, 0);
calCam.lookAt(CAL_AT);
...
// ---------------------------------------------------------------------------
const expo = { evSet: 11, evEff: 11, delta: 0, meterDelta: 0, gain: 1, fwc: 48000, K: 1, blurPx: 0, shakePx: 0, limited: '' };
...
// the bench shakes unless the damping is on; with the stabiliser the camera cancels most of it
...
function autoExposure(dt) {
  if (!state.modules.brain) { expo.limited = ''; return; }
...
function inPhoto(p, margin = 1.3) { _pp.copy(p).project(photoCam); return Math.abs(_pp.x) < margin && Math.abs(_pp.y) < margin && _pp.z < 1; }
function updateExposure() {
  ...
  // sub-exposures the shutter needs: enough that nothing jumps more than ~2 px between them
  const pxPerMm = PW / fm.w;
  let blur = 0;
  for (const s of SUBJECTS) if (s.speed && inPhoto(s.at)) blur = Math.max(blur, Opt.motion(s.speed, s.at.x, f, state.t) * pxPerMm);
  if (SUB.windmill && inPhoto(SUB.windmill.at)) blur = Math.max(blur, Opt.motion(8, SUB.windmill.at.x, f, state.t) * pxPerMm);
  expo.blurPx = blur;
...
function renderPhoto(tau) {
  ...
  const sig = [tau, f, N, s, t, K, state.hold, state.format, lightScale, lightVersion, state.trainSpeed].join();
...
function exposeScene(tau, K, t, f, N, s, fm) {
  ...
  const kPx = ((f * f) / (N * s * 10)) / fm.w * PW * 0.5;
  ...
  animate(tau);
  aimPhotoCam(tau);
  const nearMax = kPx * Math.max(0, (s - NEAREST) / NEAREST);
...
function developPhoto(f, N, s, fm) {
  ...
  displayP.u.uCocOk.value = ((fm.coc * (s * 10 - f)) / (s * 10)) / fm.w * PW * 0.5;
  // the light on the sensor: brighter with a wider aperture, nothing else
  sensorU.uGain.value = 2 ** (0.8 * (state.sceneEV - state.nd - Math.log2(N * N * 250)));
  sensorU.uUseBlur.value = su.uUseBlur.value;
  sensorU.uCrop.value.set(fm.w / 36, fm.h / 24);
  renderer.setRenderTarget(null);
}
...
function readBacks(now) {
  ...
  if (!readBusy.hist && now - histAt > 160 && state.bigView && state.overlays.hist) {
...
const screenShader = (flip) => new THREE.ShaderMaterial({
  ...
  fragmentShader: `uniform sampler2D tImg; uniform vec2 uFlip; varying vec2 vUv;
    void main(){ vec2 uv = mix(vUv, 1.0 - vUv, uFlip); gl_FragColor = vec4(pow(texture2D(tImg, uv).rgb, vec3(2.2)) * 0.85, 1.0); }`,
});
// which way a plane's texture has to be flipped to read upright to someone facing it
function uprightFlip(mesh) {
  mesh.updateMatrixWorld(true);
  const n = V3(0, 0, 1).transformDirection(mesh.matrixWorld), u = V3(1, 0, 0).transformDirection(mesh.matrixWorld), v = V3(0, 1, 0).transformDirection(mesh.matrixWorld);
  const right = V3(0, 1, 0).cross(n);
...
const sensorU = {
  ...
  uCrop: { value: new THREE.Vector2(1, 1) }, uFlip: { value: new THREE.Vector2(0, 0) },
};
bodySensor.material = new THREE.ShaderMaterial({
  uniforms: sensorU,
  vertexShader: 'varying vec2 vUv; void main(){ vUv = uv; gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0); }',
  fragmentShader: `
    uniform sampler2D tSharp, tBlur; uniform float uUseBlur, uGain; uniform vec2 uCrop, uFlip;
    varying vec2 vUv;
    void main() {
      vec2 q = (vUv - 0.5) / uCrop + 0.5;               // the part of the sensor this format uses
      vec2 uv = mix(q, 1.0 - q, uFlip);
      vec3 c = vec3(0.035, 0.04, 0.05);
      if (all(greaterThanEqual(q, vec2(0.0))) && all(lessThanEqual(q, vec2(1.0)))) {
        vec4 s = texture2D(tSharp, uv);
        vec3 lin = s.rgb;
        if (uUseBlur > 0.5) { vec4 b = texture2D(tBlur, uv); lin = mix(lin, b.rgb, smoothstep(0.6, 1.6, max(abs(s.a), b.a))); }
        c = min(lin * uGain * 1.6, vec3(1.3));
        float n = fract(sin(dot(floor(vUv * vec2(720.0, 480.0)), vec2(12.9898, 78.233))) * 43758.5453);
        c *= 0.95 + 0.05 * n;
      }
      vec2 e = abs(q - 0.5) * 2.0;
      float frame = smoothstep(0.985, 1.0, max(e.x, e.y)) * (1.0 - smoothstep(1.0, 1.02, max(e.x, e.y)));
      c += vec3(0.5, 0.89, 1.0) * frame * 1.4;
      gl_FragColor = vec4(c, 1.0);
    }`,
});
{
